        ...
        try:
            ...
            if cal_word_infl < 0:
                ...
                if config.influence.grads_path is not None and len(config.influence.grads_path) != 0:
                    if config.influence.RapidGrad.enable and isinstance(config.influence.RapidGrad.RapidGrad_K, int):
                        grad_path_name = config.influence.grads_path + f"{real_id}.pt"#f"/train_grad_K{config.influence.RapidGrad.RapidGrad_K}_{real_id}.pt"
                    else:
                        grad_path_name = config.influence.grads_path + f"/{real_id}.pt"#f"/train_grad_{real_id}.pt"
