...
def build_experiments():
    ...
    Total: 2 pre_unlearning + 2 loss_types × 2 datasets × 2 selections = 10
    """
    experiments = []

    # # 1. Pre-unlearning baseline (one per dataset — adjust if you have only one)
    # for dataset in DATASETS:
    #     experiments.append({
    #         'loss_type'   : 'pre_unlearning',
    #         'dataset'     : dataset,
    #         'selection'   : 'none',
    #         'adaptor_path': f'{ADAPTOR_BASE}/qwen_{dataset}', #llama_{dataset}
    #         'save_dir'    : None,  # not used for pre_unlearning
    #         'forget_path' : FORGET_PATHS[dataset],
    #         'test_path'   : TEST_PATHS[dataset],
    #     })

    # 2. Fine-tuned unlearning models  (2 × 2 × 2 = 8)
